...
def get_epl_upcoming_all(team_ids: list) -> dict:
    """
    Fetch upcoming EPL matches for all teams at once.
    Returns {game_id: match} deduped across all teams.
    """
    seen = {}
...
def get_mlb_upcoming_all(team_ids: list) -> dict:
    """
    Fetch upcoming MLB games for all teams at once.
    Returns {game_id: game} deduped across all teams.
    """
    today   = datetime.date.today()
    ...
    for team_id in team_ids:
        try:
            ...
            for g in games:
                if g.get("status") in MLB_PRE_GAME_STATUSES:
                    seen[g["game_id"]] = g
        except Exception as e:
            print(f"[MLB API] Upcoming error for team {team_id}: {e}")
    return seen
...
SPORT_API_HANDLERS = {
    "epl": {
        ...
        "get_upcoming_all": get_epl_upcoming_all,
    ...
    "mlb": {
        ...
        "get_upcoming_all": get_mlb_upcoming_all,
...
def check_pre_match(users: list, sent_per_user: dict, sport_key: str) -> bool:
    """
    Fetches all upcoming games for all relevant teams in one pass,
    deduplicates by game ID, then processes per-team per-user.
    """
    cfg      = SPORT_CONFIG[sport_key]
    ...
    # Build team → users mapping
    teams_to_users = {}
    for u in sport_users:
        teams_to_users.setdefault(u[field], []).append(u)

    # Collect all relevant team IDs and fetch games in one pass
    relevant_team_ids = [
        team_ids[t] for t in teams_to_users if t in team_ids
    ]
    all_upcoming = handlers["get_upcoming_all"](relevant_team_ids)
    ...
    for team_name, team_users in teams_to_users.items():
        ...
        # Filter games relevant to this team
        team_games = [
            g for g in all_upcoming.values()
            if g.get("home_id") == team_id
            or g.get("away_id") == team_id
            or g.get("homeTeam", {}).get("id") == team_id
            or g.get("awayTeam", {}).get("id") == team_id
        ]

        for event in team_games:
